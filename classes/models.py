    ...
    def _unified_callback(self, model, where):
        ...
        if where == GRB.Callback.MIPSOL:
            ...
            if self.decomposition or self.model_type == 'B&S':
                ...
                # each arc at m_ij, a valid lower bound on that arc's distance, so
                # every (d_ij - m_ij) term is non-negative. A negative value can only
                # mean the subproblem objective was read wrongly, and it is exactly
                # the case that silently defeats the violation test below (theta has
                # lb 0, so theta < sub_obj is false for any negative sub_obj and no
                # cut is ever added). Fail loudly instead of deflating the bound.
                # The extended master subtracts current_estimation, where a negative
                # value is legitimate, so the check is scoped to non-extended.
